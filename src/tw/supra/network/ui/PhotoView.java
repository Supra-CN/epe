...
import tw.supra.network.toolbox.ImageLoader.ImageListener;
...
public class PhotoView extends NetworkImageView implements OnGestureListener,
	...
	/**
	 * Touch slop used to determine if this double tap is valid for starting a
	 * scale or should be ignored.
	 */
	...
	/**
	 * The original matrix for this image; used to reset any transformations
	 * applied by the user
	 */
	...
	/**
	 * Actual crop size; may differ from {@link #sCropSize} if the screen is
	 * smaller
	 */
	...
	// To support zooming
	/**
	 * When {@code true}, a double tap scales the image by
	 * {@link #DOUBLE_TAP_SCALE_FACTOR}
	 */
	private boolean mDoubleTapToZoomEnabled = true;
	/**
	 * When {@code true}, prevents scale end gesture from falsely triggering a
	 * double click.
	 */
	private boolean mDoubleTapDebounce;
	/**
	 * When {@code false}, event is a scale gesture. Otherwise, event is a
	 * double touch.
	 */
	...
	// Convenience fields
	// These are declared here not because they are important properties of the
	// view. Rather, we
	// declare them here to avoid object allocation during critical graphics
	// operations; such as
	// layout or drawing.
	...
	/**
	 * X and Y coordinates for the current down event. Since mDoubleTapOccurred
	 * only contains the information that there was a double tap event, use
	 * these to get the secondary tap information to determine if a user has
	 * moved beyond touch slop.
	 */
	...
	private boolean scale(MotionEvent e) {
		...
		if (mDoubleTapToZoomEnabled && mTransformsEnabled && mDoubleTapOccurred) {
			if (!mDoubleTapDebounce) {
				...
				mScaleRunnable.start(currentScale, targetScale, e.getX(),
						e.getY());
				handled = true;
			}
	...
	public boolean interceptMoveLeft(float origX, float origY) {
		if (!mTransformsEnabled) {
			...
			if (!mTransformsEnabled || drawWidth <= viewWidth) {
				// Allow intercept if not in transform mode or the image is
				// smaller than the view
				return false;
			} else if (transX == 0) {
				// We're at the left-side of the image; allow intercepting
				// movements to the right
				return false;
			} else if (viewWidth >= drawWidth + transX) {
				// We're at the right-side of the image; allow intercepting
	...
	/**
	 * Free all resources held by this view. The view is on its way to be
	 * collected and will not be reused.
	 */
	public void clear() {
	...
	public void bindPhoto(Bitmap photoBitmap) {
		...
		if (mDrawable != null && currentDrawableIsBitmapDrawable) {
			final Bitmap drawableBitmap = ((BitmapDrawable) mDrawable)
					.getBitmap();
			...
			changed = photoBitmap != null
					&& (mDrawable.getIntrinsicWidth() != photoBitmap.getWidth() || mDrawable
							.getIntrinsicHeight() != photoBitmap.getHeight());
	...
	/**
	 * Gets video data associated with this item. Returns {@code null} if this
	 * is not a video.
	 */
	...
	/**
	 * Returns {@code true} if the photo represents a video. Otherwise,
	 * {@code false}.
	 */
	...
	/**
	 * Returns {@code true} if the video is ready to play. Otherwise,
	 * {@code false}.
	 */
	...
	/**
	 * Hides the photo info portion of the header. As a side effect, this
	 * automatically enables or disables image transformations [eg zoom, pan,
	 * etc...] depending upon the value of fullScreen. If this is not desirable,
	 * enable / disable image transformations manually.
	 */
	...
	/**
	 * Enable or disable cropping of the displayed image. Cropping can only be
	 * enabled <em>before</em> the view has been laid out. Additionally, once
	 * cropping has been enabled, it cannot be disabled.
	 */
	public void enableAllowCrop(boolean allowCrop) {
		if (allowCrop && mHaveLayout) {
			throw new IllegalArgumentException(
					"Cannot set crop after view has been laid out");
		}
	...
	/**
	 * Gets a bitmap of the cropped region. If cropping is not enabled, returns
	 * {@code null}.
	 */
	public Bitmap getCroppedPhoto() {
		...
		final Bitmap croppedBitmap = Bitmap.createBitmap((int) CROPPED_SIZE,
				(int) CROPPED_SIZE, Bitmap.Config.ARGB_8888);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mDrawable != null) {
			...
			if (mVideoBlob != null) {
				final Bitmap videoImage = (mVideoReady ? sVideoImage
						: sVideoNotReadyImage);
			...
			// Extract the drawable's bounds (in our own copy, to not alter the
			// image)
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		if (mAllowCrop) {
			mCropSize = Math
					.min(sCropSize, Math.min(layoutWidth, layoutHeight));
			...
			// Create a crop region overlay. We need a separate canvas to be
			// able to "punch
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		if (mFixedHeight != -1) {
			super.onMeasure(widthMeasureSpec, MeasureSpec.makeMeasureSpec(
					mFixedHeight, MeasureSpec.AT_MOST));
	...
	public void invalidateDrawable(Drawable drawable) {
		// Only invalidate this view if the passed in drawable is displayed
		// within this view. If
		// another drawable is passed in, have the parent view handle
		// invalidation.
	...
	/**
	 * Enable or disable image transformations. When transformations are
	 * enabled, this view consumes all touch events.
	 */
	...
	/**
	 * Configures the bounds of the photo. The photo will always be scaled to
	 * fit center.
	 */
	private void configureBounds(boolean changed) {
		...
		// We need to do the scaling ourself, so have the drawable use its
		// native size.
	...
	/**
	 * Generates the initial transformation matrix for drawing. Additionally, it
	...
	private void generateMatrix() {
		...
		if (fits && !mAllowCrop) {
			...
			RectF scaledDestination = new RectF((vwidth / 2)
					- (dwidth * mMaxInitialScaleFactor / 2), (vheight / 2)
					- (dheight * mMaxInitialScaleFactor / 2), (vwidth / 2)
					+ (dwidth * mMaxInitialScaleFactor / 2), (vheight / 2)
					+ (dheight * mMaxInitialScaleFactor / 2));
			if (mTempDst.contains(scaledDestination)) {
				mMatrix.setRectToRect(mTempSrc, scaledDestination,
						Matrix.ScaleToFit.CENTER);
			} else {
				mMatrix.setRectToRect(mTempSrc, mTempDst,
						Matrix.ScaleToFit.CENTER);
			}
		}
	...
	private float getScale() {
		return FloatMath.sqrt((float) Math.pow(
				getValue(mMatrix, Matrix.MSCALE_X), 2)
	 ...
	 *
	 * The given scale is capped so that the resulting scale of the image always
	 * remains between {@link #mMinScale} and {@link #mMaxScale}.
	 *
	 * The scaled image is never allowed to be outside of the viewable area. If
	 * the image is smaller than the viewable area, it will be centered.
	 *
	 * @param newScale
	 *            the new scale
	 * @param centerX
	 *            the center horizontal point around which to scale
	 * @param centerY
	 *            the center vertical point around which to scale
	 */
	 ...
	 *
	 * This method will not allow the image to be translated outside of the
	 * visible area.
	 *
	 * @param tx
	 *            how many pixels to translate horizontally
	 * @param ty
	 *            how many pixels to translate vertically
	 * @return {@code true} if the translation was applied as specified.
	 *         Otherwise, {@code false} if the translation was modified.
	 */
	private boolean translate(float tx, float ty) {
		...
		if (mAllowCrop) {
			// If we're cropping, allow the image to scroll off the edge of the
			// screen
	...
	private void snap() {
		...
		final float translateY;
		if (b - t < maxBottom - maxTop) {
			// Image is shorter than view; translate to the bottom edge of the
			// view
			translateY = maxTop + ((maxBottom - maxTop) - (b + t)) / 2;
		} else if (t > maxTop) {
		...
		if (Math.abs(translateX) > SNAP_THRESHOLD
				|| Math.abs(translateY) > SNAP_THRESHOLD) {
	 ...
	 *
	 * @param degrees
	 *            how many degrees to rotate the image, positive rotates
	 *            clockwise
	 * @param animate
	 *            if {@code true}, animate during the rotation. Otherwise, snap
	 *            rotate.
	 */
	...
	private void initialize() {
		...
		if (!sInitialized) {
			...
			sCropSize = resources
					.getDimensionPixelSize(R.dimen.photo_crop_width);

			sCropDimPaint = new Paint();
			sCropDimPaint.setAntiAlias(true);
			sCropDimPaint.setColor(resources
					.getColor(R.color.photo_crop_dim_color));
			sCropDimPaint.setStyle(Style.FILL);

			sCropPaint = new Paint();
			sCropPaint.setAntiAlias(true);
			sCropPaint.setColor(resources
					.getColor(R.color.photo_crop_highlight_color));
			sCropPaint.setStyle(Style.STROKE);
			sCropPaint.setStrokeWidth(resources
					.getDimension(R.dimen.photo_crop_stroke_width));
		...
		mQuickScaleEnabled = ScaleGestureDetectorCompat
				.isQuickScaleEnabled(mScaleGetureDetector);
	...
	private static class ScaleRunnable implements Runnable {
		 ...
		 */
		public boolean start(float startScale, float targetScale,
		...
		/**
		 * Stops the animation in place. It does not snap the image to its final
		 * zoom.
		 */
		public void stop() {
			mRunning = false;
		...
		public void run() {
			...
			// Stop when done
			if (newScale == mTargetScale
					|| (mZoomingIn == (newScale > mTargetScale))) {
	...
	private static class TranslateRunnable implements Runnable {
		...
		public void run() {
			...
			long now = System.currentTimeMillis();
			float delta = (mLastRunTime != NEVER) ? (now - mLastRunTime) / 1000f
					: 0f;
			final boolean didTranslate = mHeader.translate(mVelocityX * delta,
					mVelocityY * delta);
			mLastRunTime = now;
			// Slow down:
	...
	private static class SnapRunnable implements Runnable {
		...
		public void run() {
			...
			if (delta >= SNAP_DURATION) {
				...
				if (Math.abs(transX) > Math.abs(mTranslateX)
						|| transX == Float.NaN) {
					transX = mTranslateX;
				}
				if (Math.abs(transY) > Math.abs(mTranslateY)
						|| transY == Float.NaN) {
					transY = mTranslateY;
				}
			}
