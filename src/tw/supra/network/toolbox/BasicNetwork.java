...
import tw.supra.network.RetryPolicy;
		...
		while (true) {
			...
			try {
				...
				if (httpResponse.getEntity() != null) {
					responseContents = entityToBytes(request,
				...
				if (httpResponse != null) {
					...
					throw new NoConnectionError(new NetworkResponse(-1, null,
							null, false));
				}
				...
				if (responseContents != null) {
					 ...
					 * attemptRetryOnException("auth", request, new
					 * AuthFailureError(networkResponse)); } else { // TODO:
					 * Only throw ServerError for 5xx status codes. throw new
