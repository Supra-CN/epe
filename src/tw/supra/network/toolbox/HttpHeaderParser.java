...
public class HttpHeaderParser {
	...
	public static Cache.Entry parseCacheHeaders(NetworkResponse response) {
		...
		if (null != response) {
			...
			// Cache-Control takes precedence over an Expires header, even if
			// both exist and Expires
			// is more restrictive.
			if (hasCacheControl) {
	...
	public static Cache.Entry parseIgnoreCacheHeaders(NetworkResponse response) {
		...
		final long cacheHitButRefreshed = 0; // 3 * 60 * 1000; // in 3 minutes
												// cache will be hit, but also
												// refreshed on background
		final long cacheExpired = 0; // 24 * 60 * 60 * 1000; // in 24 hours this
