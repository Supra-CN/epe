...
public class ImageRequest extends Request<BitmapDrawable> {
	...
	public ImageRequest(String url, Resources resources,
			...
			Response.Listener<BitmapDrawable> listener, int maxWidth,
	...
	@Override
	protected Response<BitmapDrawable> parseNetworkResponse(
	...
	private Response<BitmapDrawable> doVideoFileParse() {
		...
		if (mMaxWidth == 0 && mMaxHeight == 0) {
			...
			decodeOptions.inSampleSize = ImageUtils.findBestSampleSize(
	...
	private Response<BitmapDrawable> doContentParse() {
		...
		if (mContentResolver == null) {
			return Response.error(new ParseError(
					"Content Resolver instance is null"));
		}
		final String requestUrl = getUrl();
		// Remove the 'content://' prefix
		// final String imageData = requestUrl.substring(10,
		// requestUrl.length());
		...
		if (mMaxWidth == 0 && mMaxHeight == 0) {
			bitmap = ImageUtils.decodeStream(mContentResolver, imageUri,
					decodeOptions);
			...
			decodeOptions.inSampleSize = ImageUtils.findBestSampleSize(
					actualWidth, actualHeight, desiredWidth, desiredHeight);
			Bitmap tempBitmap = ImageUtils.decodeStream(mContentResolver,
					imageUri, decodeOptions);
	...
	private static void copyOptions(BitmapFactory.Options from,
			BitmapFactory.Options to) {
	...
	@TargetApi(11)
	private static void copyOptionsHoneycomb(BitmapFactory.Options from,
			BitmapFactory.Options to) {
		copyOptionsGingerbreadMr1(from, to);
	...
	@TargetApi(10)
	private static void copyOptionsGingerbreadMr1(BitmapFactory.Options from,
			BitmapFactory.Options to) {
		copyOptionsFroyo(from, to);
		to.inPreferQualityOverSpeed = from.inPreferQualityOverSpeed;
	}

	private static void copyOptionsFroyo(BitmapFactory.Options from,
			BitmapFactory.Options to) {
