...
import tw.supra.network.ui.AnimateImageView;
import tw.supra.network.ui.RecyclingBitmapDrawable;
...
public class ImageLoader {
	 ...
	 */
	private final ArrayMap<String, BatchedImageRequest> mInFlightRequests = new ArrayMap<String, BatchedImageRequest>();

	/** HashMap of the currently pending responses (waiting to be delivered). */
	private final ArrayMap<String, BatchedImageRequest> mBatchedResponses = new ArrayMap<String, BatchedImageRequest>();
	 ...
	 * error image.
	 *
	 * This version returns one specifically for AnimateImageView which allows
	 * it to animate the image changing
	 * 
	 * @param defaultImageResId
	 *            Default image resource ID to use, or 0 if it doesn't exist.
	 * @param errorImageResId
	...
	public static ImageListener getImageListener(final AnimateImageView view,
		...
		return new ImageListener() {
			...
			public void onResponse(ImageContainer response, boolean isImmediate) {
				if (response.getBitmap() != null) {
					/**
					 * Animate the image changing only if the image was fetched
					 * from network
					 */
	...
	public ImageContainer get(String requestUrl, ImageListener imageListener,
		...
		// network and
		// track it.
		Request<?> newRequest = new ImageRequest(requestUrl, mResources,
				mContentResolver, new Listener<BitmapDrawable>() {
					@Override
	...
	public ImageContainer set(String requestUrl, ImageListener imageListener,
		...
		// The bitmap did not exist in the cache, fetch it!
		ImageContainer imageContainer = new ImageContainer(drawable,
		...
		Response<?> response = Response.success(bitmap,
				HttpHeaderParser.parseBitmapCacheHeaders(bitmap));
		getCache().put(requestUrl, response.cacheEntry);

		/*
		 * Response<?> response = Response.success(bitmap,
		 * HttpHeaderParser.parseBitmapCacheHeaders(bitmap)); Entry cache =
		 * getCache().get(requestUrl); cache.data = response.cacheEntry.data;
	...
	/**
	 * Set a {@link Resources} instance if you need to support resource uris for
	 * loading images
	 * 
	 * @param resources
	 *            {@link Resources} instance for loading images. Get from
	 *            {@link Context#getResources()}
	 */
	...
	/**
	 * Set a {@link ContentResolver} instance if you need to support content
	 * uris for loading images
	 * 
	 * @param resources
	 *            {@link ContentResolver} instance for loading images. Get from
