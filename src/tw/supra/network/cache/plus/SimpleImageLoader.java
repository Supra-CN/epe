...
package tw.supra.network.cache.plus;
...
import tw.supra.network.cache.DiskLruBasedCache;
...
import tw.supra.network.toolbox.BasicNetwork;
...
public class SimpleImageLoader extends ImageLoader {
	...
	public SimpleImageLoader(FragmentActivity activity) {
		super(newRequestQueue(activity, null), BitmapImageCache
				.getInstance(activity.getSupportFragmentManager()), activity
				.getResources());
	}
	...
	public SimpleImageLoader(FragmentActivity activity,
		...
		super(newRequestQueue(activity, imageCacheParams), BitmapImageCache
				.getInstance(activity.getSupportFragmentManager(),
						imageCacheParams), activity.getResources());
	}
	 ...
	 */
	public SimpleImageLoader(FragmentActivity activity,
			int defaultPlaceHolderResId, ImageCacheParams imageCacheParams) {
		super(newRequestQueue(activity, imageCacheParams), BitmapImageCache
				.getInstance(activity.getSupportFragmentManager(),
						imageCacheParams), activity.getResources());
		mPlaceHolderDrawables = new ArrayList<Drawable>(1);
		mPlaceHolderDrawables.add(defaultPlaceHolderResId == -1 ? null
				: getResources().getDrawable(defaultPlaceHolderResId));
	}
	...
	public SimpleImageLoader(Context context, int defaultPlaceHolderResId,
			ImageCacheParams imageCacheParams) {
		super(newRequestQueue(context, imageCacheParams), BitmapImageCache
				.getInstance(null, imageCacheParams), context.getResources());
		mPlaceHolderDrawables = new ArrayList<Drawable>(1);
		mPlaceHolderDrawables.add(defaultPlaceHolderResId == -1 ? null
				: getResources().getDrawable(defaultPlaceHolderResId));
	}
	...
	public void invalidate(String key) {
		final String cacheKey = getCacheKey(key, mMaxImageWidth,
				mMaxImageHeight);
	...
	public SimpleImageLoader setMaxImageSize(int maxImageWidth,
			int maxImageHeight) {
	...
	public ImageContainer get(String requestUrl, ImageView imageView,
		...
		return get(requestUrl, imageView,
				mPlaceHolderDrawables != null ? mPlaceHolderDrawables.get(0)
						: null, maxImageWidth, maxImageHeight);
	}
	...
	public ImageContainer get(String requestUrl, ImageView imageView,
		...
		return get(
				requestUrl,
				imageView,
				mPlaceHolderDrawables != null ? mPlaceHolderDrawables
						.get(placeHolderIndex) : null, mMaxImageWidth,
				mMaxImageHeight);
	}
	...
	public ImageContainer get(String requestUrl, ImageView imageView,
		...
		if (requestUrl == null || !requestUrl.equals(recycledImageUrl)) {
			...
			if (requestUrl != null) {
				...
				imageContainer = get(
						requestUrl,
						getImageListener(getResources(), imageView,
								placeHolder, mFadeInImage), maxWidth, maxHeight);
	...
	// Set
	public ImageContainer set(String requestUrl, ImageView imageView,
			Bitmap bitmap) {
		return set(requestUrl, imageView, 0, bitmap);
	}

	public ImageContainer set(String requestUrl, ImageView imageView,
			int placeHolderIndex, Bitmap bitmap) {
		return set(
				requestUrl,
				imageView,
				mPlaceHolderDrawables != null ? mPlaceHolderDrawables
	...
	public ImageContainer set(String requestUrl, ImageView imageView,
			Drawable placeHolder, Bitmap bitmap) {
		return set(requestUrl, imageView, placeHolder, mMaxImageWidth,
				mMaxImageHeight, bitmap);
	}

	public ImageContainer set(String requestUrl, ImageView imageView,
			Drawable placeHolder, int maxWidth, int maxHeight, Bitmap bitmap) {
		...
		if (requestUrl != null) {
			...
			imageContainer = set(
					requestUrl,
					getImageListener(getResources(), imageView, placeHolder,
							mFadeInImage), maxWidth, maxHeight, bitmap);
			// Store request in ImageView tag
			imageView.setTag(imageContainer);
		} else {
			if (!(imageView instanceof PhotoView)) {
		...
		return new ImageListener() {
			...
			public void onResponse(ImageContainer response, boolean isImmediate) {
				...
				if (response.getBitmap() != null) {
					...
					if (imageView instanceof PhotoView) {
						setPhotoImageBitmap((PhotoView) imageView,
								response.getBitmap(), resources, fadeInImage
										&& !isImmediate);
					} else {
						setImageBitmap(imageView, response.getBitmap(),
								resources, fadeInImage && !isImmediate);
					}
				} else {
					if (!(imageView instanceof PhotoView)) {
						imageView.setImageDrawable(placeHolder);
	...
	private static RequestQueue newRequestQueue(Context context,
		...
		Network network = new BasicNetwork(
				Utils.hasHoneycomb() ? new HurlStack() : new HttpClientStack(
						AndroidHttpClient.newInstance(NetUtils
		...
		if (null != imageCacheParams) {
			...
			cache = new DiskLruBasedCache(Utils.getDiskCacheDir(context,
					CACHE_DIR));
		}
	...
	private static void setImageBitmap(final ImageView imageView,
			final BitmapDrawable bitmapDrawable, Resources resources,
			boolean fadeIn) {
	 ...
	 */
	@TargetApi(Build.VERSION_CODES.HONEYCOMB_MR1)
	private static void setPhotoImageBitmap(final PhotoView imageView,
			final BitmapDrawable bitmapDrawable, Resources resources,
			boolean fadeIn) {
		// If we're fading in and on HC MR1+
		if (fadeIn && Utils.hasHoneycombMR1()) {
