...
public class ImageUtils {
	...
	private static final Pattern BASE64_IMAGE_URI_PATTERN = Pattern
			.compile("^(?:.*;)?base64,.*");
	 ...
	 *            The local URI
	 * @param maxSize
	 *            The maximum size (either width or height)
	 * @return The new bitmap or null
	 */
	public static Bitmap decodeStream(final ContentResolver resolver,
			final Uri uri, final int maxSize) {
		Bitmap result = null;
		final InputStreamFactory factory = createInputStreamFactory(resolver,
				uri);
		try {
			...
			opts.inSampleSize = Math
					.max(bounds.x / maxSize, bounds.y / maxSize);
	 ...
	 */
	public static Bitmap decodeStream(final ContentResolver resolver,
			final Uri uri, BitmapFactory.Options opts) {
		Bitmap result = null;
		final InputStreamFactory factory = createInputStreamFactory(resolver,
				uri);
		try {
	...
	public static Bitmap decodeStream(InputStream is, Rect outPadding,
		...
		try {
			...
			Log.e(TAG,
					"ImageUtils#decodeStream(InputStream, Rect, Options) threw an OOME",
					oome);
			return null;
		}
	}
	 ...
	 *
	 * @param factory
	 *            Used to create input streams that holds the raw data to be
	 ...
	 *         be returned (in opts.outWidth and opts.outHeight)
	 */
	public static Bitmap decodeStream(final InputStreamFactory factory,
			final Rect outPadding, final BitmapFactory.Options opts)
		...
		try {
			...
			if (is != null && originalBitmap == null
					&& !opts.inJustDecodeBounds) {
				Log.w(TAG,
						"ImageUtils#decodeStream(InputStream, Rect, Options): "
			...
			Log.e(TAG,
					"ImageUtils#decodeStream(InputStream, Rect, Options) threw an IOE",
					ioe);
			return null;
		} finally {
			if (is != null) {
	 ...
	 *
	 * @param factory
	 *            Used to create the InputStream.
	 *
	 ...
	 * For example, one pass may load EXIF orientation, and the second pass may
	 * do the actual Bitmap decode.
	 */
	public interface InputStreamFactory {

		/**
		 * Create a new InputStream. The caller of this method must be able to
		 * read the input stream starting from the beginning.
		 * 
		 * @return
		 */
	...
	private static class BaseInputStreamFactory implements InputStreamFactory {
		...
		public BaseInputStreamFactory(final ContentResolver resolver,
				final Uri uri) {
			mResolver = resolver;
			mUri = uri;
		}
	...
	private static class DataInputStreamFactory extends BaseInputStreamFactory {
		...
		public DataInputStreamFactory(final ContentResolver resolver,
				final Uri uri) {
			super(resolver, uri);
		}
		...
		private byte[] parseDataUri(final Uri uri) {
			...
			try {
				if (ssp.startsWith(BASE64_URI_PREFIX)) {
					final String base64 = ssp.substring(BASE64_URI_PREFIX
							.length());
					return Base64.decode(base64, Base64.URL_SAFE);
				} else if (BASE64_IMAGE_URI_PATTERN.matcher(ssp).matches()) {
					final String base64 = ssp.substring(ssp
							.indexOf(BASE64_URI_PREFIX)
							+ BASE64_URI_PREFIX.length());
