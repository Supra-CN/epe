...
public class Account extends PersistableObj {
    private static final String LOG_TAG = Account.class.getSimpleName();
    public static final String ANONYMOUS_UID = Type.ANONYMOUS.name();
    ...
    public static Account build(String passport) {
        Account user;
        if (!isLegalNormalPassport(passport)) {
            user = new Account();
        } else {
            user = new Account(passport);
        }
        try {
            user.reset();
        } catch (IOException e) {
    ...
    private Account() {
        UID = Type.ANONYMOUS.name();
        TYPE = Type.ANONYMOUS;
        init();
    }

    private Account(String passport) {
        UID = passport;
        TYPE = Type.NORMAL;
        init();
    }
    ...
    private void init() {
        if (!UID.equals(getData().getPrefStr(PreferencesUser.UID, UID))) {
            getData().putPrefStr(PreferencesUser.UID, UID);
        }
        if (!TYPE.name().equals(getData().getPrefStr(PreferencesUser.TYPE, TYPE.name()))) {
            getData().putPrefStr(PreferencesUser.TYPE, TYPE.name());
        }
    }
    ...
    public Cursor queryUserInfo() {
        ...
        String selection = String.format("%s = '%s'", TableAccounts.Columns.UID, UID);
    ...
    public void putToken(String token) {
        getData().putPrefStr(PreferencesUser.TOKEN, token);
    }

    public void putPPToken(String ppToken) {
        getData().putPrefStr(PreferencesUser.PP_TOKEN, ppToken);
    }

    public String getPassProt() {
        return getPrefStringByKey(PreferencesUser.UID);
    }

    public String getToken() {
        return getPrefStringByKey(PreferencesUser.TOKEN);
    }

    public String getPPToken() {
        return getPrefStringByKey(PreferencesUser.PP_TOKEN);
    }
    ...
    protected String generateString(String key) {
        if (PreferencesUser.SIG.equals(key)) {
            return generateSIG();
        } else if (PreferencesUser.TOKEN.equals(key)) {
            return "";
        } else if (PreferencesUser.PP_TOKEN.equals(key)) {
            return "";
        } else {
            return "";
        }
    ...
    public boolean equals(Object o) {
        return (o instanceof Account && UID.equals(((Account) o).UID));
    }

    @Override
    public String toString() {
        return String.format("User: passport = %s,  type = %s", UID, TYPE.name());
    }

    @Override
    protected void onReset() {
        String selection = String.format("%s = '%s'", TableAccounts.Columns.UID, UID);
        Cursor c = CommonData.getInstance().getDb()
                .query(TableAccounts.NAME, null, selection, null, null, null, null);
        c.moveToFirst();
        bindData(c);
        c.close();
    }
    ...
    protected void onFlush() {
        String table = TableAccounts.NAME;
        String whereClause = String.format("%s = '%s'", TableAccounts.Columns.UID, UID);
    ...
    private void bindData(Cursor c) {
        ...
        setLastLogin(DBUtils.getStrByCol(c, TableAccounts.Columns.LAST_LOGIN));
        ...
        setLoginCount(DBUtils.getIntByCol(c, TableAccounts.Columns.LOGIN_COUNT));
        setProvince(DBUtils.getStrByCol(c, TableAccounts.Columns.PROVINCE));
        setMessageTotal(DBUtils.getIntByCol(c, TableAccounts.Columns.MESSAGE_TOTAL));
        setFriendsTotal(DBUtils.getIntByCol(c, TableAccounts.Columns.FRIENDS_TOTAL));
        setFollowersTotal(DBUtils.getIntByCol(c, TableAccounts.Columns.FOLLOWERS_TOTAL));
    ...
    private ContentValues buileValues() {
        ...
        stringMap.put(TableAccounts.Columns.NICK_NAME, mNickName);
    ...
    public static boolean isLegalNormalPassport(String passport) {
        if (!TextUtils.isEmpty(passport) && !Account.ANONYMOUS_UID.equals(passport)) {
            return true;
        } else {
            return false;
        }
