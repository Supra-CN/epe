...
public class RequestMaster extends EpeJsonRequest<MasterInfo> {
	private static final String LOG_TAG = RequestMaster.class.getSimpleName();

	public RequestMaster(NetWorkHandler<MasterInfo> eventHandler,
			MasterInfo info) {
	...
	protected void parseJsonResponse(JSONObject response) throws JSONException {
		...
		INFO.resultJoList = JsonUtils.getJaSafely(response, "division_t");
		INFO.ERROR_CODE.addDyingMsg("response : " + response);
	}
