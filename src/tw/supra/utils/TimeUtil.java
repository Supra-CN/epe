...
public class TimeUtil {
    ...
    public static String formatTimeWithCountDown(Context context, long timeStamp) {
        ...
        long diff = currentTime - timeStamp;
        if (diff < MINUTE) {
            return context.getString(R.string.time_some_seconds_ago, (diff / SECOND));
        } else if (diff < HOUR) {
            return context.getString(R.string.time_some_minutes_ago, (diff / MINUTE));
        } else if (diff < DAY) {
            return context.getString(R.string.time_some_hours_ago, (diff / HOUR));
        } else if (diff < 2 * DAY) {
            ...
            return context.getString(R.string.time_some_week_ago, (diff / WEEK));
        } else if (diff < YEAR) {
            return new SimpleDateFormat("MM-dd HH:mm").format(new Date(timeStamp));
        } else {
            return new SimpleDateFormat("yyyy-MM-dd").format(new Date(timeStamp));
